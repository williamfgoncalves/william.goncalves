...
function seriesInvalidas(series){
    var data = new Date();
    let seriesinvalidasObjeto = series.filter(serie => {
    let campoInvalido = Object.values(serie).some(s => s === null || typeof s === 'undefined');
        return campoInvalido;
    });
    let serieInvalidaAno = series.filter( series => series.anoEstreia > data.getFullYear());
    return seriesinvalidasObjeto.map(s => s.titulo) + (" - ")+serieInvalidaAno.map(s => s.titulo).join(" - ");
}


//Exercicio 2
function filtrarSeriesPorAno(ano, series){
    let serieAno = series.filter( series => series.anoEstreia === ano);
    return "Series Invalidas: " + serieAno.map(series => series.titulo).join(" - ");
}

//Exercicio 3
function mediaDeEpisodios(series){
    let mediaEp = series.map(s => s.numeroEpisodios).reduce((numeroEpisodios,c) => c +=numeroEpisodios)/series.length;
    return mediaEp;
}

//Exercicio 4
function procurarPorNome(series, nome){
    let atorSerie = series.map(series => series.elenco.includes(nome));
    let souAtor = atorSerie.some(atorSerie => atorSerie === true) 
    return souAtor;
}
