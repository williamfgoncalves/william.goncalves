Create table Cidade -- Criação da tabela Cidade
(
...
Create table Associado -- Criação da tabela Associado
(
  ...
  IDCidade       int,
	 ...
	 constraint FK_Associado foreign key (IDCidade) -- Chave estrangeira da tabela Associado que referencia cidade
	    references Cidade (IDCidade)  -- Criação da referencia
);
...
select * into copiaCidade from Cidade;  -- Copiando todos os dados da tabela cidade para a tabela copia Cidade.
 -- lembrando que esta copia nao traz consigo as definicões de chaves primarias e estrangeiras.
